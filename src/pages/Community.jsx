...
const EventsTab = ({ navigate }) => {
  ...
  return (
    <div className="tab-content">
      ...
      <div className="cards-list">
        {events.map(event => (
          <div key={event.id} className="event-card" onClick={() => navigate(`/community/event/${event.id}`)} style={{ cursor: 'pointer' }}>
            ...
            <button
              className={`event-join-btn ${registeredMap[event.id] ? 'btn-registered' : 'btn-join-event'}`}
              onClick={() => toggle(event.id)}
            >
              {registeredMap[event.id] ? '✓ Registered' : 'Join event'}
            </button>
          </div>
        ))}
      </div>
...
const Community = () => {
  ...
  const renderTab = () => {
    switch (activeTab) {
      ...
      case 'events':   return <EventsTab navigate={navigate} />;
